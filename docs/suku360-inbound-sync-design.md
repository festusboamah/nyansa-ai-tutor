...
**Status: v1 (read-only roster pull) shipped.** This document was originally written as design-only, before Suku360 was confirmed to be a real, live system (it is — a separate Django+DRF codebase, same owner, at `suku360.com`). Section 11 below records what was actually built once the real Suku360 codebase existed to design against, including where reality diverged from this document's original speculation. Sections 1-10 are kept as-written for the historical record of the reasoning that held up (most of it did) and what turned out to need correcting.

The outbound half (Nyansa exposing approved evidence/mastery/risk-signal data *to* Suku360) was already shipped before this — see `integrations/` app and `docs/delivery-roadmap.md` Phase 6. Nyansa's evidence actually landing in Suku360's UI is still not implemented: Suku360's own `learner_success`/`intelligence` domains have no ingestion point for an external signal today, so that direction needs new Suku360-side modeling, not just a new endpoint — deferred to a future slice.
...
## 11. What was actually built (v1)

Suku360 turned out to have **zero server-to-server authentication anywhere** (session-cookie auth only, system-wide) and **no existing whole-school roster endpoint** for a machine caller — the closest thing, `SyncBootstrapView`, is session-authenticated as one specific teacher and scoped to that teacher's own classes. So this slice built real infrastructure on *both* repos, not just the Nyansa consumer side §1-9 assumed would be enough once a contract existed.

**On Suku360** (`domains/integrations/`, new app): `PartnerCredential` (school FK, `partner_name`, SHA-256 `token_hash`, `scope`, `is_active`) plus `PartnerTokenAuthentication` (a new DRF authentication class — the first non-session auth mechanism in the whole codebase) and `HasPartnerCredential`. One endpoint, `GET /api/v1/integrations/roster/` — deliberately **not** nested under `schools/<school_pk>/` like the rest of the API, since the caller has no session/Membership and therefore no client-supplied school id to even consider trusting; school scope comes entirely from the credential. Credential issuance is Django-admin-only for v1 (a `save_model`/`rotate_token` admin action generates and shows the raw token once). `docs/19_SECURITY_TESTING_THREAT_MODEL.md` got a new threat scenario entry per that doc's own stated maintenance rule.

**On Nyansa**: `suku360_id` added to `schools.School`, `schools.SchoolMembership` (school-scoped unique), `academics.SchoolClass` (school-scoped unique) as §4 proposed. `SubjectOffering` did **not** get its own `suku360_id` — resolving §9's third open question: Suku360's `TeachingAssignment` already includes the teacher, so the natural match is Nyansa's `TeacherAssignment` (offering + teacher), not the teacher-less `SubjectOffering`. `TeacherAssignment.suku360_id` is deliberately **not unique**: Suku360 scopes a teaching assignment per academic year, Nyansa scopes it per term (via `offering`), so one Suku360 assignment legitimately produces one Nyansa row per term in that year. `integrations.SyncBatch`/`SyncRecord` match §8's proposed shape closely (batch status, one record per entity with `CREATED`/`UPDATED`/`UNCHANGED`/`ERROR`), wrapped in one `@transaction.atomic` per batch. `integrations.Suku360RosterCredential` stores the token *Suku360* issued to Nyansa (plaintext, not hashed — the opposite direction from `IntegrationCredential`, which stores a hash of a token *Nyansa* issued). `integrations/suku360_sync.py::pull_roster(school)` does the actual upsert, reusing `roster_import.py`'s `set_unusable_password()`/unique-username conventions for any newly-seen student or teacher. `pull_suku360_roster` is a plain management command (no Celery/cron infra exists on either side), matching `billing`'s `generate_license_invoices` convention exactly.

§9's other two open questions (read-only-once-synced enforcement in Nyansa's UI; reconciliation for pre-existing schools with no `suku360_id`) remain genuinely open — this slice performs the sync but doesn't yet lock synced fields from local editing, and a first sync against a school with pre-existing manually-created classes/memberships will create duplicates rather than matching by name. Both are real follow-up work, not silently solved.

Confirmed empty-handed: Suku360 already has its own fully-built strand-mastery engine (`domains/academics`) and `learner_success`/`intelligence` layers, computed entirely from its own data, with no field on any model there for recording which external tool contributed a signal. Nyansa's evidence/mastery data reaching Suku360's UI needs new modeling on Suku360's side before it's possible at all — not attempted in this slice.

## 12. Premium gating + automated handoff (v1.1)

Two follow-ups landed before either v1 commit was pushed: the integration is now Suku360-Premium-only, and the credential handoff/first sync no longer requires a human to copy a token by hand.

**On Suku360**: `SchoolBillingPlan.tier` (`STANDARD`/`PREMIUM`, default `STANDARD`) — deliberately on the existing billing model rather than a new entitlement system, since it's already "what a school is entitled to." `RosterView.get` re-checks `school.billing_plan.tier == PREMIUM` on **every request** (not just at credential-creation time), matching this codebase's existing live-check idiom for `School.trial_ends_at` — a school downgraded mid-sync loses access immediately, proven live by downgrading a test school and re-hitting the endpoint with its still-active token (403, not the 200 it got before the downgrade). `PartnerCredentialAdminForm` blocks creating/activating a credential for a non-Premium school or one missing the new `nyansa_school_slug` field; `rotate_token` got the same two checks added after testing surfaced that it bypassed the form entirely (it mutates the queryset directly, no `ModelForm.clean()` involved). `per_student_amount` is *not* defaulted at the field level — an early attempt to default it to GHS 8.00 (the going per-child Premium price) broke an existing test that relies on `None` meaning "no rate configured yet"; the suggested price lives only as a form-initial value on `SchoolBillingPlanInline` (`SchoolBillingPlan.DEFAULT_PER_STUDENT_AMOUNT`), never silently populated on plain `.create()`.

Automation: `PartnerCredentialAdmin.save_model`/`rotate_token` now POST the new token to Nyansa (`{nyansa_school_slug, suku360_school_id, token, base_url}`, HMAC-SHA512-signed with a new shared `SUKU360_WEBHOOK_SECRET`, same scheme as the Paystack webhooks) via `domains/integrations/webhook_push.py`. This push is registered with `transaction.on_commit(...)`, not called inline — calling it inline meant Nyansa's callback fetch of the roster (using the brand-new token) could race a still-uncommitted row inside admin's own `transaction.atomic()`, reproduced live as a spurious 401 before the fix. On Nyansa, `integrations/suku360_webhook.py` + `POST /suku360/webhook/credential/` verifies the signature, upserts `Suku360RosterCredential` by school slug, and runs `pull_roster` synchronously — a pull failure here doesn't block saving the credential itself. Proven end-to-end with real HTTP calls between two live dev servers: admin form submission → signed webhook → credential saved on Nyansa → roster pulled automatically, zero manual steps. `render.yaml` documents (commented out) a periodic re-sync cron, flagged as needing Render's paid tier.

The one manual step left, by design: which Suku360 school maps to which Nyansa school (`nyansa_school_slug`) has no automatable source of truth yet — two separate tenant records in two separate systems with nothing to look it up by until the first sync happens.
