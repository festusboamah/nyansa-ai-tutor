...
To create the synthetic demonstration school, configure `DEMO_ADMIN_USERNAME`, `DEMO_ADMIN_PASSWORD`, and optionally `DEMO_ADMIN_EMAIL` as secret environment variables on the Render web service. Use a unique password of at least 12 characters. The startup command runs `seed_demo` after migrations; it safely creates or updates one demonstration school with administrator, teacher, guardian, and student personas, a class roster, subject assignments, approved grades, attendance, a published report, guardian access, fees, an explainable risk signal, and a versioned lesson plan awaiting administrator review. The personas use the same private demo password and the usernames `demo-teacher`, `demo-guardian`, `demo-student`, and `demo-student-2`. The command refuses to run outside demo mode and never stores credentials in GitHub.
